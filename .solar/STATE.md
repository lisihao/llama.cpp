...
## Tier A: 微优化系列 — ⚠️ 2026-03-15 重新测试后废弃

**重新测试发现 (正确参数 -t 4)**：所有微优化实际效果 <1%，远低于预期

| # | 优化方案 | 声称效果 | 实际效果 | 状态 |
|---|---------|---------|---------|------|
| A1 | MoE Kernel Fusion (SOFT_MAX+ARGSORT+GET_ROWS) | +12.5% | **-0.17%** | ❌ 虚假优化 |
| A2 | Q5_K Branchless Dequant | +2-5% | - | ❌ 已废弃（已切换到 Q4_K_M） |
| A3 | MoE Threshold=16 | +1.7% | **+0.53%** | ⚠️ 效果被夸大 |
| A4 | SWIGLU 向量化 | - | **+0.06%** | ✅ 符合预期但 ROI 极低 |
| A5+ | 其他微优化 | - | - | ❌ **已废弃** |

**废弃原因**：
1. 真实累计效果 ~0.4%（vs 声称 14%+）
2. 性能主要来自正确参数 (-t 4, +11%) 而非算法优化
3. 微优化投入产出比过低
4. 应转向系统性优化 (Paged Attention, Flash Attention 2)

**测试教训**: 参数配置 > 算法优化，测试方法论至关重要
